...
class KsGlobalDiscountInvoice(models.Model):
    ...
    def _compute_amount(self):
        result = super(KsGlobalDiscountInvoice, self)._compute_amount()
        ...
        for rec in self:
            if rec.move_type in ['in_refund', 'out_refund']:
                ...
                rec.amount_after_discount = rec.amount_total_signed
    # @api.multi
        ...
        for rec in self:
            if rec.ks_global_discount_type == "amount":
                ...
                if param == 'False':
                 ...
                 rec.amount_tax = sum(t.get('amount', 0.0) for t in
                                         self.invoice_line_ids.mapped('tax_ids').compute_all(rec.amount_untaxed).get(
                                             'taxes', []))
