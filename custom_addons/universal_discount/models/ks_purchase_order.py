...
class KSGlobalDiscountPurchases(models.Model):
    ...
    #     for rec in self:
    #         context = eval(ks_res['context'])
    #         context.update({'default_ks_global_discount_rate':rec.ks_global_discount_rate,
    #                         'default_ks_global_discount_type':rec.ks_global_discount_type
    #                         })
    #         ks_res['context']=json.dumps(context)
    #     return ks_res
    ...
    def ks_calculate_discount(self):
        for rec in self:
            if rec.ks_global_discount_type == "amount":
                ...
                param = self.env['ir.config_parameter'].sudo().get_param('is_tax_calculated_before_discount')
                if param == 'False':
                 rec.amount_tax = sum(t.get('amount', 0.0) for t in
                    ...
                    rec.amount_tax = sum(t.get('amount', 0.0) for t in
                                         self.order_line.mapped('taxes_id').compute_all(rec.amount_untaxed).get(
                                             'taxes', []))
