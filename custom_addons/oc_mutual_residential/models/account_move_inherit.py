    ...
    def _post(self, soft=True):
        for rec in self:
            if rec.move_type == 'entry':
                return super(AccountMoveInherit, self)._post()
        ...
        for move in to_post:
            ...
            if move:#move._auto_compute_invoice_reference():
                to_write = {
                    'payment_reference': move._get_invoice_computed_reference(),
                    'monitoring_period_from' : str(move.monitoring_period_from),
                    'monitoring_period_to': str(move.monitoring_period_to),
                    'line_ids': []
                }
                for line in move.line_ids.filtered(lambda line: line.account_id.user_type_id.type in ('receivable', 'payable')):
                    # original code
                    dibators_label = to_write['payment_reference']+' - Monitoring Period From '+to_write['monitoring_period_from']+' to '+to_write['monitoring_period_to']
                    to_write['line_ids'].append((1,line.id,{'name':dibators_label}))
